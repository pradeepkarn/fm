...
function updatePage()
{
    if (isset($_POST['page_id']) && isset($_POST['update_page'])) {
        ...
        if (isset($_POST['duration'])) {
            $db->insertData['duration'] = sanitize_remove_tags($_POST['duration']);
        }
        if (isset($_POST['duration_unit'])) {
            $db->insertData['duration_unit'] = sanitize_remove_tags($_POST['duration_unit']);
        }
        if (isset($_POST['service_cat_id'])) {
            $db->insertData['service_cat_id'] = sanitize_remove_tags($_POST['service_cat_id']);
        }
...
function addContent($type="service")
{
    if (isset($_POST['add_new_content'])) {
        ...
        if (isset($_POST['company_id'])) {
            $db->insertData['company_id'] = $_POST['company_id'];
        }else{
            $_SESSION['msg'][] = "Please provide salon id";
        ...
        $db->insertData['content_type'] = "service";
        $db->insertData['created_by'] =  isset($_POST['created_by'])?$_POST['created_by']:$_SESSION['user_id'];
