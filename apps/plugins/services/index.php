...
switch ($path) {
    ...
    case "admin/{$plugin_dir}/delete/{$GLOBALS['url_last_param']}":
        if (is_superuser()==false) {
            ...
            $serv = getData('content',$GLOBALS['url_last_param']);
            $salon_id = $serv['company_id'];
            $vendor_id = $serv['created_by'];
            if(delContent($id=$GLOBALS['url_last_param']) != false){
                echo js_alert("Deleted Successfully");
                header("Location:/".home."/admin/{$plugin_dir}/services-by-salon/?vendor_id=$vendor_id&salon_id=$salon_id");
            }
            else{
    ...
    if (count($url)>=3) {
        ...
        if ($url[2]=='add-new-item-ajax') {
            ...
            if (filter_var($pageid,FILTER_VALIDATE_INT)) {
                echo js_alert('added');
                $home = home;
                $vendorid = isset($_POST['created_by'])?$_POST['created_by']:$_SESSION['user_id'];
                $salon_id = isset($_POST['company_id'])?$_POST['company_id']:0;
                echo js("location.href='/$home/admin/services/services-by-salon/?vendor_id={$vendorid}&salon_id={$salon_id}';");
            }
            return;
        }
        ...
        if ("{$url[1]}/{$url['2']}"=="{$plugin_dir}/delete-content-details") {
            if (isset($_POST['content_details_delete_id'])) {
                ...
                if($listObj->destroy($_POST['content_details_delete_id'])==true){
                    echo js("location.href='/$home/admin/services/services-by-salon/?vendor_id=160&salon_id=359';");
                }
                else{
