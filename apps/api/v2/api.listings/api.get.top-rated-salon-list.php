...
if ($user) {
    ...
    $rating_list = $ratingobj->filter_index(array('content_group' => 'star-rating'),"desc");
    
   
// Sort the array by content ID
usort($rating_list, function ($a, $b) {
    return $a["content_id"] <=> $b["content_id"];
});

// Calculate average rating for each content ID and make the content IDs unique
$ratings = array();
$averages = array();
foreach ($rating_list as $item) {
    ...
    if (!isset($ratings[$contentId])) {
        $ratings[$contentId] = array();
    }

    $ratings[$contentId][] = $detail;
}

foreach ($ratings as $contentId => $detailArray) {
    $average = array_sum($detailArray) / count($detailArray);
    $averages[] = array(
...
// print_r($averages);
    $salons = array();
    foreach ($averages as $fv) {
        $sl = getData('content', $fv['content_id']);
        $avg_rt_pt = $fv['average_rating'];
        if ($sl) {
